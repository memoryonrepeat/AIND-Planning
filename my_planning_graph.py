...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            candidate_action_node = PgNode_a(action)
            if candidate_action_node.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(candidate_action_node)
                candidate_action_node.parents.update(candidate_action_node.parents)
    ...
    def add_literal_level(self, level):
        ...
        for prev_action_node in self.a_levels[level-1]:
            for candidate_state_node in prev_action_node.effnodes:
                self.s_levels[level].add(candidate_state_node)
                prev_action_node.children.add(candidate_state_node)
                candidate_state_node.parents.add(prev_action_node)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return bool((set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)) | (set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)) | (set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos)) | (set(node_a2.action.effect_add) & set(node_a1.action.precond_neg)))
